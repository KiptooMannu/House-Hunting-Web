...
export const getAdminStats = async () => {
  ...
  // 2. Revenue Growth (Last 6 Months)
  ...
  // 3. Listings by County
  const regionalDistribution = await db.select({
    name: locations.county,
    value: sql<number>`count(*)::int`
  })
  .from(locations)
  .innerJoin(houses, eq(houses.locationId, locations.locationId))
